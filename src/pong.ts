...
function pong() {
  ...
  const mainObservable = mainInterval
    .takeUntil(mainInterval.filter(_ => pongStats.score1 == pongStats.maxScore ||  // takeUntil one of the player's reaches the maxScore
  ...
  .map(({x, y, r, xSpeed, ySpeed}) => ({x,y,r, xSpeed, ySpeed, 
      ...
      paddleWidth: parseInt(rightPaddle.attr('width'))})) //both paddles have same height and widths
  .filter(({x,y,r, xSpeed, ySpeed, paddleHeight, paddleWidth}) => 
  ...
  mainObservable
    .filter(({y,r,ySpeed}) =>  isCollision(y-r, 0, ySpeed) ||     // if the ball is close enough to the edge (in the range of ySpeed). 
  ...
  mainObservable
    ...
    .filter(({y,paddleHeight}) => isBetween(y, Math.floor(0+paddleHeight/2), Math.floor(svg.getBoundingClientRect().height - paddleHeight),0)) // filtering the cy coordinate of ball when it is `paddleHeight/2` units away from both the top and bottom bounds
    .subscribe(({y, paddleHeight}) => leftPaddle.attr('y', y - Math.floor(paddleHeight/2))) // mapping the balls cy to the paddle's centre y coordinate

// resetting the game 
mainObservable
.filter(({x,xSpeed,r}) => isCollision(x-r, 0, xSpeed) || // reset if the ball's left most edge is close enough to the 0
  isCollision(x+r, Math.floor(svg.getBoundingClientRect().width), xSpeed)) // reset if the ball's right most edge is close enough to the right bound
.forEach(({x,pongStats}) => (x < svg.getBoundingClientRect().width/2 ? scored(pongStats.score1,++pongStats.score2,pongStats.maxScore) : scored(++pongStats.score1,pongStats.score2,pongStats.maxScore))) // if the game was resetted then check if the ball is on the left side then player2 scored, else player1 did
.subscribe(({xSpeed, ySpeed}) => ball.attr('cx', getRandomBetween(400,500)).attr('cy', getRandomBetween(250,350)).attr('xSpeed', -1*xSpeed).attr('ySpeed', -1*ySpeed)) // spawn the ball randomly and reverse the x and y speeds, just for fun!
}
