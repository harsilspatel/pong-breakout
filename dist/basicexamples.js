...
function mousePosObservable() {
  const pos = document.getElementById("pos"),
    o = Observable.fromEvent(document, "mousemove").map(
      ({ clientX, clientY }) => ({ x: clientX, y: clientY })
    );
  o.map(({ x, y }) => `${x},${y}`).subscribe(s => (pos.innerHTML = s));
  o.filter(({ x }) => x > 400).subscribe(_ => pos.classList.add("highlight"));
  o.filter(({ x }) => x <= 400).subscribe(_ =>
    pos.classList.remove("highlight")
  );
}
function animatedRectTimer() {
  ...
  const animate = setInterval(
    () => rect.attr("x", 1 + Number(rect.attr("x"))),
    10
  );
...
function dragRectEvents() {
  ...
  rect.elem.addEventListener("mousedown", e => {
    const xOffset = Number(rect.attr("x")) - e.clientX,
      yOffset = Number(rect.attr("y")) - e.clientY,
      moveListener = e => {
        rect.attr("x", e.clientX + xOffset).attr("y", e.clientY + yOffset);
      },
      done = () => {
...
function dragRectObservable() {
  ...
  const mousemove = Observable.fromEvent(svg, "mousemove"),
    mouseup = Observable.fromEvent(svg, "mouseup"),
...
function dragRect(svg, rect) {
  ...
  return rect
    ...
    .flatMap(({ xOffset, yOffset }) =>
      mousemove.takeUntil(mouseup).map(({ clientX, clientY }) => ({
        x: clientX + xOffset,
        y: clientY + yOffset
      }))
    )
    .map(({ x, y }) => rect.attr("x", x).attr("y", y));
}
